...
// MARK: - 순서대로 불러와짐 - 속도는 아래 코드보다 조금 느림
import UIKit
...
extension ListViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "segue_detail" {
            ...
            // 사용자가 클릭한 행을 찾아낸다. (테이블 뷰에서 몇 번재 셀인지 인덱스 값을 받아옴)
...
//            }
//
//            let log = NSString(data: data, encoding: String.Encoding.utf8.rawValue) ?? "데이터가 없습니다."
//            NSLog("API Result=\(log)")
//
//            do {
...
//                    mvo.rating = (r["ratingAverage"] as! NSString).doubleValue
//
//                    self.getThumbnailImage(mvo, completion: { image in
//                        if let image = image {
//                            mvo.thumbnailImage = image
//                            self.list.append(mvo)
//                            self.tableView.reloadData()
//                        }
//                    })
...
//    }
//
//    func getThumbnailImage(_ mvo: MovieVO, completion: @escaping (UIImage?) -> Void) {
//        if let savedImage = mvo.thumbnailImage {
//            completion(savedImage)
//        } else {
//            DispatchQueue.global().sync {
//                if let url = URL(string: mvo.thumnail!), let imageData = try? Data(contentsOf: url), let image = UIImage(data: imageData) {
//                    DispatchQueue.main.async {
//                        completion(image)
//                    }
//                } else {
